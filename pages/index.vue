  ...
  <div class="flex flex-col md:flex-row h-screen font-sans">
    ...
    <div class="flex-grow bg-gray-200 flex items-center justify-center p-4 relative overflow-hidden">
      <div v-if="isLoading" class="flex flex-col items-center text-center">
        ...
        <p class="text-gray-600 font-semibold">กำลังวิเคราะห์ใบหน้า...</p>
        <p class="text-sm text-gray-500">กรุณารอสักครู่</p>
      </div>
      <div v-else-if="errorMessage" class="text-center bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg max-w-md">
      ...
      <div v-else-if="imagePreviewUrl" class="relative w-full h-full flex items-center justify-center">
        ...
        <svg v-if="analysisResult" 
          ...
          <g v-if="displayOptions.landmarks">
            <circle v-for="(point, name) in analysisResult[0].faceLandmarks"
                    :key="name"
                    :cx="point.x"
                    :cy="point.y"
                    r="4" 
          ...
          <line v-if="displayOptions.eyeLine"
                :x1="analysisResult[0].faceLandmarks.pupilLeft.x"
                :y1="analysisResult[0].faceLandmarks.pupilLeft.y"
                :x2="analysisResult[0].faceLandmarks.pupilRight.x"
                :y2="analysisResult[0].faceLandmarks.pupilRight.y"
                stroke="#3b82f6"
                stroke-width="3"
                stroke-linecap="round"
          ></line>
          
          <line v-if="displayOptions.smileLine"
                :x1="analysisResult[0].faceLandmarks.mouthLeft.x"
                :y1="analysisResult[0].faceLandmarks.mouthLeft.y"
                :x2="analysisResult[0].faceLandmarks.mouthRight.x"
                :y2="analysisResult[0].faceLandmarks.mouthRight.y"
                stroke="#22c55e"
                stroke-width="3"
                stroke-linecap="round"
          ></line>

          <line v-if="displayOptions.midline && facialMidlinePoints"
                :x1="facialMidlinePoints.top.x"
                :y1="facialMidlinePoints.top.y"
                :x2="facialMidlinePoints.bottom.x"
                :y2="facialMidlinePoints.bottom.y"
                stroke="#ef4444"
                stroke-width="3"
...
const analysisResult = ref<any>(null); // เราจะใช้ any ไปก่อนเพื่อความง่าย
...
const displayOptions = ref({
    landmarks: true, eyeLine: true,
    smileLine: true, midline: true,
});
...
const analyzePhoto = async (file: File) => {
    ...
    try {
        // ใช้ $fetch ของ Nuxt เพื่อเรียก API ของเราเอง
